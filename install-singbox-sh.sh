...
configure_relay_outbound() {
    if [ "${INSTALL_MODE:-server}" != "relay" ]; then
        return 0
    fi

    echo ""
    info "=== 线路机模式：添加落地机出站 ==="
    read -p "是否现在添加落地机出站并生成线路机路由？(y/N): " ADD_RELAY_OUT

    if [[ ! "$ADD_RELAY_OUT" =~ ^[Yy]$ ]]; then
        info "已跳过添加落地机出站"
        return 0
    fi
    ...
    if [[ ! "$LANDING_LINK" =~ ^ss:// ]]; then
        err "当前版本先支持 SS 落地机链接"
        exit 1
    fi

    local raw link_no_scheme main_part userinfo server_part decoded
    raw="${LANDING_LINK#ss://}"
    raw="${raw%%#*}"
    raw="${raw%%\?*}"
    ...
    if [ -z "$LANDING_METHOD" ] || [ -z "$LANDING_PASSWORD" ] || [ -z "$LANDING_SERVER" ] || [ -z "$LANDING_PORT" ]; then
        err "解析落地机链接失败"
        exit 1
    fi
    ...
    if sing-box check -c "$CONFIG_PATH" >/dev/null 2>&1; then
        ...
        info "线路机模式配置完成：所有入站流量将转发到落地机出站 landing-out"
    else
        err "配置校验失败，已保留备份文件，请检查 $CONFIG_PATH"
        exit 1
    fi
}
